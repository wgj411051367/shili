...
{
    return [NSString stringWithFormat:@"sender = %@ text = %@", _sender.id , _msgText];
}
...
- (NSData *)packToSendData
{
    ...
    if ([NSJSONSerialization isValidJSONObject:post])
    {
        ...
        if(error)
        {
            NSLog(@"[%@] Post Json Error: %@", [self class], post);
            return nil;
        }
        
        NSLog(@"AVIMCMD content is %@", post);
        return data;
    }
    else
    {
        NSLog(@"[%@] AVIMCMD is not valid: %@", [self class], post);
        return nil;
    }
}
...
- (NSString *)description
{
    return [NSString stringWithFormat:@"sender = %@ action = %d", _sender.id, (int)_userAction];
}
@end
