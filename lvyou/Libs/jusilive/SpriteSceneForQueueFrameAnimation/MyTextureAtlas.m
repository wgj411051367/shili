...
@interface ImageMapEntry : NSObject {
    __weak ImageMap *_parent;
...
@property (readonly) SKTexture *texture;
- (instancetype) initWithDict:(NSDictionary *)dict parent:(ImageMap *)parent;
@end
...
- (instancetype)initWithFile:(NSString *)path
{
    ...
    if (self) {
        ...
        for (NSDictionary *item in [mapping objectForKey:@"images"]) {
            id result = [[ImageMap alloc] initWithDict:item parent:self];
...
- (instancetype) initWithDict:(NSDictionary *)dict parent:(MyTextureAtlas *)parent
{
    ...
    if (self) {
        ...
        NSMutableArray *mutableEntries = [NSMutableArray arrayWithCapacity:0];
        for (NSDictionary *item in [dict objectForKey:@"subimages"]) {
            ImageMapEntry *entry = [[ImageMapEntry alloc] initWithDict:item parent:self];
...
{
    SKTexture *result = nil;
    for (ImageMapEntry *entry in _entries) {
...
//    UIGraphicsBeginImageContextWithOptions(newSize, NO, 0.0);//18.5.22此处的参数是精确的像素大小。
    UIGraphicsBeginImageContext(newSize); // 2018.8.31修改
...
- (SKTexture *)texture
{
    if (_texture == nil) {
        if (self.image) {
            _texture = [SKTexture textureWithImage:self.image];//2018.1.8修改
        }
    }
    return _texture;
}
...
- (SKTexture *) texture
{
    if (_texture == nil) {
        ...
        NSString *platform=[SharedAppDelegate getCurrentModel]; //18.7.9添加 
        ...
        if (!_textureRotated) {
            ...
            adjPoint = CGPointMake((_textureRect.size.width - sourceSize.width) / 2 + _spriteOffset.x,
                                   (_textureRect.size.height - sourceSize.height) / 2 + _spriteOffset.y);
            cropOffset = CGPointMake(adjPoint.x/_textureRect.size.width, adjPoint.y/_textureRect.size.height);
            
        } else {
            sourceSize = CGSizeMake(_spriteSourceSize.height, _spriteSourceSize.width);
            cropScale = CGPointMake(_textureRect.size.height / sourceSize.height,_textureRect.size.width / sourceSize.width);
            adjPoint = CGPointMake((_textureRect.size.width - sourceSize.width) / 2 + _spriteOffset.y,
                                   (_textureRect.size.height - sourceSize.height) / 2 + _spriteOffset.x);
            cropOffset = CGPointMake(adjPoint.y/_textureRect.size.height,adjPoint.x/_textureRect.size.width);
