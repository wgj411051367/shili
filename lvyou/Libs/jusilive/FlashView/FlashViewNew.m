...
-(BOOL) innerInit{
    ...
    if (!filePath) {
        ...
        if (!filePath) {
            filePath = [NSString stringWithFormat:@"%@/%@.flajson", mFlashAnimDir, mFlashName];
            if ([mFileManager fileExistsAtPath:filePath]) {
                mFileType = FlashViewFileTypeDocument;
                mFileDataType = FlashViewFileDataTypeJson;
            }else{
                filePath = [NSString stringWithFormat:@"%@/%@.flabin", mFlashAnimDir, mFlashName];
                if ([mFileManager fileExistsAtPath:filePath]) {
                    mFileType = FlashViewFileTypeDocument;
...
-(void) setScaleMode:(FlashViewScaleMode)mode andDesignResolution:(CGSize)resolution{
    ...
    CGFloat wRate = self.frame.size.width / resolution.width;
    CGFloat hRate = self.frame.size.height / resolution.height;
    switch (mode) {
...
-(NSData *)readData{
    ...
    switch (mFileType) {
        case FlashViewFileTypeDocument:
            data = [NSData dataWithContentsOfFile:[NSString stringWithFormat:@"%@/%@%@", mFlashAnimDir, mFlashName, mFileDataType == FlashViewFileDataTypeJson ? @".flajson" : @".flabin"]];
            break;
...
-(UIImage *)readImage:(NSString *)path{
    switch (mFileType) {
        case FlashViewFileTypeDocument:
            return [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"%@/%@/%@", mFlashAnimDir, mFlashName, path]];
        case FlashViewFileTypeResource:
...
-(BOOL) parseBinFile{
    ...
    switch (mFileType) {
        case FlashViewFileTypeDocument:
            imagePath = [NSString stringWithFormat:@"%@/%@", mFlashAnimDir, mFlashName];
            break;
        case FlashViewFileTypeResource:
