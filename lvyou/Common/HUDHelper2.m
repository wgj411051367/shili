...
@implementation HUDHelper2

static HUDHelper2 *_instance = nil;


+ (HUDHelper2 *)sharedInstance
{
    @synchronized(_instance)
    {
        if (_instance == nil)
        {
            _instance = [[HUDHelper2 alloc] init];
        }
        return _instance;
    }
}
...
{
    [HUDHelper2 alert:msg cancel:@"确定"];
}
//+ (void)alert:(NSString *)msg action:(CommonVoidBlock)action
//{
//    [HUDHelper2 alert:msg cancel:@"确定" action:action];
//}
+ (void)alert:(NSString *)msg cancel:(NSString *)cancel
{
    [HUDHelper2 alertTitle:@"提示" message:msg cancel:cancel];
}
//+ (void)alert:(NSString *)msg cancel:(NSString *)cancel action:(CommonVoidBlock)action
//{
//    [HUDHelper2 alertTitle:@"提示" message:msg cancel:cancel action:action];
//}
...
{
    UIView *inView = view ? view : SharedAppDelegate.window;
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIView *inView = SharedAppDelegate.window;
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:inView];
        if (![NSString isEmpty:msg])
        {
...
- (void)tipMessage:(NSString *)msg delay:(CGFloat)seconds completion:(void (^)())completion
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithWindow:SharedAppDelegate.window];
